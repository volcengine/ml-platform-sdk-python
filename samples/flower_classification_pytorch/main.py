...
def main(config):
    tos_source_list = [
        's3://{}/flower-classification/photos/train_manifest.json'.format(
            BUCKET),
        's3://{}/flower-classification/photos/val_manifest.json'.format(BUCKET)
    ]

    train_manifest_info = get_manifest(tos_source_list[0],
                                       './train_manifest.json')
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        train_one_epoch(config, model, criterion, data_loader_train, optimizer,
                        epoch, lr_scheduler)
        if dist.get_rank() == 0 and (epoch % config.SAVE_FREQ == 0 or
                                     epoch == (config.TRAIN.EPOCHS - 1)):
            save_checkpoint(config, epoch, model_without_ddp, max_accuracy,
                            optimizer, lr_scheduler, logger, BUCKET)
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch,
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            if config.DEVICE == 'cuda':
                memory_used = torch.cuda.max_memory_allocated() / (1024.0 *
                                                                   1024.0)

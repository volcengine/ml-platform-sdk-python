...
os.system(
    "wget https://ml-platform-public-examples-cn-beijing.tos-cn-beijing.volces.com/bert/util/transformers.tar.gz"
)
...
TOS_MODEL_SOURCE = f"https://ml-platform-public-examples-cn-beijing.tos-cn-beijing.volces.com/bert/model/{BERT_BASE_MODEL}.tar.gz"
model_file = TOS_MODEL_SOURCE.split("/")[-1]
model_file = f"{BASE_DIR}/{model_file}"
os.system(
    f"wget -O {model_file} {TOS_MODEL_SOURCE} && tar -zxvf {model_file} -C {MODEL_PATH}/"
)

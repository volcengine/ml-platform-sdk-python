...
def window_partition(x, window_size):
    ...
    x = tf.reshape(x,
                   shape=[
                       -1, H // window_size, window_size, W // window_size,
                       window_size, C
                   ])
...
def window_reverse(windows, window_size, H, W, C):
    x = tf.reshape(windows,
                   shape=[
                       -1, H // window_size, W // window_size, window_size,
                       window_size, C
                   ])
    x = tf.transpose(x, perm=[0, 1, 3, 2, 4, 5])
    ...
    def call(self, x, mask=None):
        ...
        if mask is not None:
            ...
            attn = tf.reshape(attn,
                              shape=[-1, nW, self.num_heads, N, N]) + tf.cast(
                                  tf.expand_dims(tf.expand_dims(mask, axis=1),
                                                 axis=0), tf.float32)
...
class PatchEmbed(tf.keras.layers.Layer):
    ...
    def call(self, x):
        ...
        x = tf.reshape(x,
                       shape=[
                           -1, (H // self.patch_size[0]) *
                           (W // self.patch_size[0]), self.embed_dim
                       ])
    ...
    def __init__(self,
        ...
        self.basic_layers = tf.keras.Sequential([
            BasicLayer(
                ...
                drop_path_prob=dpr[sum(depths[:i_layer]):sum(depths[:i_layer +
                                                                    1])],
                norm_layer=norm_layer,
...
def SwinTransformer(model_name='swin_tiny_224',
                    ...
                    cfgs=CFGS):
    cfg = cfgs[model_name]
    net = SwinTransformerModel(model_name=model_name,
...
# 改为TOS
#    pretrained_ckpt = TOSClient.get_object(tfrecord, swin_large_224.tgz)
    else:
